...
def move(pose):
    ...
    [dtrans, drot1, drot2] = pose[:3] + motion_noise
    ...
    lspeed, rspeed, (phi_l, phi_r) = get_wheel_speeds(x_true)
    
    u = [np.linalg.norm(x_true[:2] - np.array([pose_x,pose_y])), phi_l, phi_r]
...
def main():
    global cov, c_prob, cov_new, Rt, Qt, mu, mu_new, u
